...
func TestCommandStatusWithMeetings(t *testing.T) {
	...
	if err := s.AddParticipant(ctx, &store.Participant{
		MeetingID: "m1",
		UserName:  "Alice",
		JoinTime:  time.Now(),
	}); err != nil {
...
func TestCommandFilters(t *testing.T) {
	...
	// Add a filter
	if err := s.CreateFilter(ctx, &store.MeetingFilter{TenantID: "T-CMD", Pattern: "Standup"}); err != nil {
		t.Fatal(err)
	}
...
func TestCommandSetSuffixTenantDefault(t *testing.T) {
	...
	tenant, err := s.GetTenant(ctx, "T-CMD")
	if err != nil {
		t.Fatal(err)
	}
...
func TestCommandSetLinkFilterOverride(t *testing.T) {
	...
	// Create a filter
	if err := s.CreateFilter(ctx, &store.MeetingFilter{TenantID: "T-CMD", Pattern: "Retro"}); err != nil {
		t.Fatal(err)
	}
...
func TestCommandSettings(t *testing.T) {
	...
	// Set some defaults
	if err := s.UpdateTenantDefaults(ctx, "T-CMD", "the meeting", true); err != nil {
		t.Fatal(err)
	}
